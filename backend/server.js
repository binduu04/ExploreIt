...
// Serve frontend static files (Vite build in /public) 
...
//     await storeUserTokens(req.user.uid, tokens);
...
// app.get('/api/calendar/permissions', verifyFirebaseToken, async (req, res) => {
//   try {
//     const tokens = await getUserTokens(req.user.uid);
//     const hasPermissions = tokens !== null;
...
// const ensureValidTokens = async (userId) => {
...
//       await updateUserTokens(userId, credentials);
...
//     const tokens = await ensureValidTokens(req.user.uid);
    
//     console.log('Using valid tokens for calendar operations');
...
//     if (error.message.includes('Token refresh failed') || error.message.includes('No tokens found')) {
...
//       `https://api.openweathermap.org/geo/1.0/direct?q=${destination.replace(/\s+/g, ',')}&limit=1&appid=${WEATHER_API_KEY}`
//     );
...
//     services: {
//       gemini: !!process.env.GEMINI_API_KEY,
//       database: true // Add your database health check here if applicable
//     }
//   });
// });
